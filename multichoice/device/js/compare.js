
 // A $( document ).ready() block.
$( document ).ready(function() {
  
   //Packages Text
   var addpackpremtext = '<h4>You Get:</h4>\
    <ul>\
 	<li>Our biggest selection of channels, from live sport coverage and the latest news from around the globe, to award winning documentaries, non-stop movies, gripping series, kids edutainment and the hottest music\
       </li>\
	   <li> \
          DStv mobile free for the first device </li> \
      <li> \
	  DStv TV Guide online  \
	  </li>\
	  <li>\
	  BoxOffice PVR & online at R27/movie \
	  </li>\
	  </ul>';
	
	
	
	  var addpackextratext = '<h4>You Get:</h4>\
    <ul>\
	<li>\
 	Entertainment, news, sports and more with some of your favourite channels like BBC entertainment, Comedy Central, E! Entertainment, kykNET, Africa Magic Movies 1, FOX SD, The History Channel, TRACE Sports, Nickelodeon, VH1 Classic and Sky News<li>DStv Guide Online  </li><li>BoxOffice Online at R27/movie</li></ul>';

	
	
	
		
  var addpackcompacttext = '<h4>You Get:</h4>\
    <ul>\
	<li>\
  The latest in local and international entertainment, including Highlight Mzanis, SuperSport 3 & 4, Cartoon Network, and Disney Junior\
       </li>\
	   <li> \
        DStv Guide online </li> \
      <li> \
	BoxOffice PVR & Online at R32/movie\
	  </li>\
	  </ul>';
	
	
	
	
	
	var addpackfamtext = '<h4>You Get:</h4>\
    <ul>\
	<li>\
  	The latest news and sports highlights, your favourite décor and fashion shows, classic movies, great local content on KykNet and endless kids\' entertainment on Cartoon Network\
       <li> \
          DStv mobile free for the first device </li> \
      <li> \
	 DStv Guide Online \
	  </li>\
	  <li>\
	   BoxOffice Online at R32/movie\
	  </li>\
	  </ul>';
	
	
	
	
	var addpackAccesstext = '<h4>You Get:</h4>\
    <ul>\
	<li>\
  A selection of fantastic local and international channels, including Mzanzi Wethu, Channel O, Africa Magic and M-Net Movies Zone\
       </li>\
       <li> \
         DStv Guide online \
		   </li> \
      <li> \
	BoxOffice Online at R32/movie\
	  </li>';
	
	
	
	var addpackEasytext = '<h4>You Get:</h4>\
    <ul>\
	<li>\
  A selection of essential entertainment channels, including Africa Magic World, Mzanzi Bioskop, Soweto TV, 1KZN and Bay TV\
       </li>\
       <li> \
       DStv Guide online \
		   </li> \
      <li> \
	BoxOffice Online at R32/movie\
	  </li>\
	  </ul>';
	
	
	//Packages Text
	
	
	
	
	
	
	
	
	
	
	
	
	
	
	//Add Packages Text
   var packpremtext = ' <li>\
 	Our biggest selection of channels, from live sport coverage and the latest news from around the globe, to award winning documentaries, non-stop movies, gripping series, kids edutainment and the hottest music\
       <li> \
          DStv mobile free for the first device </li> \
      <li> \
	  DStv TV Guide online  \
	  </li>\
	  <li>\
	  BoxOffice PVR & online at R27/movie \
	  </li>';
	
	
	
	  var packextratext = '<li>\
 	Entertainment, news, sports and more with some of your favourite channels like BBC entertainment, Comedy Central, E! Entertainment, kykNET, Africa Magic Movies 1, FOX SD, The History Channel, TRACE Sports, Nickelodeon, VH1 Classic and Sky News<li>DStv Guide Online  </li><li>BoxOffice Online at R27/movie</li>';

	
	
	
		
  var packcompacttext = '<li>\
  The latest in local and international entertainment, including Highlight Mzanis, SuperSport 3 & 4, Cartoon Network, and Disney Junior\
       <li> \
        DStv Guide online </li> \
      <li> \
	BoxOffice PVR & Online at R32/movie\
	  </li>';
	
	
	
	
	
	var packfamtext = '<li>\
  	The latest news and sports highlights, your favourite décor and fashion shows, classic movies, great local content on KykNet and endless kids\' entertainment on Cartoon Network\
       <li> \
          DStv mobile free for the first device </li> \
      <li> \
	 DStv Guide Online \
	  </li>\
	  <li>\
	   BoxOffice Online at R32/movie\
	  </li>';
	
	
	
	
	var packAccesstext = '<li>\
  A selection of fantastic local and international channels, including Mzanzi Wethu, Channel O, Africa Magic and M-Net Movies Zone\
       </li>\
       <li> \
         DStv Guide online \
		   </li> \
      <li> \
	BoxOffice Online at R32/movie\
	  </li>';
	
	
	
	var packEasytext = '<li>\
  A selection of essential entertainment channels, including Africa Magic World, Mzanzi Bioskop, Soweto TV, 1KZN and Bay TV\
       </li>\
       <li> \
       DStv Guide online \
		   </li> \
      <li> \
	BoxOffice Online at R32/movie\
	  </li>';
	
	
	//Add Packages Text
	
	
	
	
	
	
	
	
	
	
	
	
	
	
	var everywhereAccesstextprem = '<h4>You Get:</h4>\
   <ul>\
   <li>\
 Watch two different channels  on two different TVs with XtraView \
       </li>\
       <li> \
       Catch Up Online \
		   </li> \
      <li> \
	Live Sports online via SuperSport Apps\
	  </li>\
    </ul>';
	
	
	
	var everywhereAccesstextextra = '<h4>You Get:</h4>\
   <ul>\
   <li>\
  Watch two different channels  on two different TVs with XtraView \
       </li>\
       <li> \
      Sports highlights online via SuperSports Apps \
		   </li> \
    </ul>';
	
	
	var everywhereAccesstextcomp = '<h4>You Get:</h4>\
   <ul>\
   <li>\
   Watch two different channels  on two different TVs with XtraView \
       </li>\
       <li> \
      Sports highlights online via SuperSports Apps\
		   </li> \
    </ul>';


  var everywhereAccesstextfam = '<h4>You Get:</h4>\
   <ul>\
   <li>\
 Watch two different channels  on two different TVs with XtraView\
       </li>\
       <li> \
       Sports highlights online via SuperSports Apps\
		   </li> \
    </ul>';
	
	
	
	
	var everywhereAccesstextaccess = '<h4>You Get:</h4>\
   <ul>\
   <li>\
     Watch two different channels  on two different TVs with XtraView \
       </li>\
       <li> \
      Sports highlights online via SuperSports Apps\
		   </li> \
    </ul>';
	
	
	
	
	var everywhereAccesstexteasyview = '<h4>You Get:</h4>\
   <ul>\
   <li>\
 Watch two different channels  on two different TVs with XtraView  \
       </li>\
       <li> \
       Sports highlights online via SuperSports Apps\
		   </li> \
      <li> \
	BoxOffice Online at R32/movie\
	  </li>\
    </ul>';
	
	
	
	
	//Decoder Text
	var decodertextprem = '<li>\
Catch Up, so you can watch movies, sports highlights, doccies and series that you missed anytime you want\
       </li>\
       <li> \
       The latest blockbusters on BoxOffice at R27 per movie\
		   </li> \
      <li> \
	HD viewing\
	  </li>\
	   <li> \
	Ultra slow-mo & rewind\
	  </li>\
	   <li> \
	Up to 200 hours of recording space \
	  </li>\
  ';
	
	
	
	var decodertextextra = '<li>\
     Record live TV\
       </li>\
       <li> \
       The latest blockbusters on BoxOffice at R27 per movie\
		   </li> ';
	
	
	var decodertextcomp = '<ul>\
   <li>\
 Record live TV  \
       </li>\
       <li> \
      The latest blockbusters on BoxOffice at R32 per movie\
		   </li> \
      <li> \
	BoxOffice Online at R32/movie\
	  </li>\
    </ul>';
	
	
	var decodertexthd = '<h4>You Get:</h4>\
   <ul id="panel2ul_comp">\
   <li>\
 recommended retail price, including installation. Optional R35pm insurance\
       </li>\
    </ul>';
	
	
	
	//Decoder Text
	
	
	
	
	var sum = 665;
	var sum2 = 1999;
	var premium =665;
	var extra = 339;
	var compact = 295;
	var family = 185;
	var access = 99;
	var easyview = 29;
	var explora = 1999;
	var hddecoder =  799;
	
	var rand = "R";
	var finalValue;
	var packageSum = document.getElementById("selectPkg").childNodes;
	var decodersSum = document.getElementById("decoderssum").childNodes;
	var totalFee = document.getElementById("totalFee").childNodes;
	var panel1 = document.getElementById("panel1_comp");
	var panel2 = document.getElementById("panel2_comp");
	var panel3 = document.getElementById("panel3_comp");
	var extrasSum = document.getElementById("extrasSum");
	var extrasSum2 = document.getElementById("extrasSum2");
	var panel2ul = document.getElementById("panel2ul");
	var perMonth = " pm";
	var Exploratxt = "Explora";
	var hddecodertxt = "HD Decoder";
	var premiumtxt = "Premium";
    var extratxt = "Extra";
    var compacttxt = "Compact";
    var familytxt = "Family";
    var accesstxt = "Access";
    var easyviewtxt = "Easy View";
	

	
    //Compare Packages
	
	$( "#premium_comp" ).click(function() {
		//Replace Device Text
		
		$('#panel2ul_comp').html(packpremtext);
		//Button disable.
		//Per month
		$('#comppm1').html(rand+premium+perMonth);
		//Permonth
		
		
		//Change Header Text and Color
		$('#dropdowntextcompack').html(premiumtxt);
		$('#dropdownMenu3').attr("style","background-color:#a5aaaf");
		
		
      });
	
	
	
	
	$( "#extra_comp" ).click(function() {

		//Change Device Panel Txt.
		$('#panel2ul_comp').html(packextratext);
		//Change Device Panel Txt.
		//Per month
		$('#comppm1').html(rand+extra+perMonth);
		//Permonth
		
		//Change Header Text and Color
		$('#dropdowntextcompack').html(extratxt);
		$('#dropdownMenu3').attr("style","background-color:#5faeab");

      });
	
	
	$( "#compact_comp" ).click(function() {
       $('#panel2ul_comp').html(packpremtext);
		//Button disable.
		//Per month
		$('#comppm1').html(rand+compact+perMonth);
		//Permonth
		
		//Change Header Text and Color
		$('#dropdowntextcompack').html(compacttxt);
		$('#dropdownMenu3').attr("style","background-color:#f7dc06");

    });
    
	
	$( "#family_comp" ).click(function() {
		
		 $('#panel2ul_comp').html(packpremtext);
		//Button disable.
		//Per month
		$('#comppm1').html(rand+family+perMonth);
		//Permonth
		
		//Change Header Text and Color
		$('#dropdowntextcompack').html(familytxt);
		$('#dropdownMenu3').attr("style","background-color:#84b341")
		
    });
	
	$( "#access_comp" ).click(function() {
     
		
		 $('#panel2ul_comp').html(packpremtext);
		//Button disable.
		//Per month
		$('#comppm1').html(rand+access+perMonth);
		//Permonth
		
		//Change Header Text and Color
		$('#dropdowntextcompack').html(accesstxt);
		$('#dropdownMenu3').attr("style","background-color:#c30336")


    });
	
	
	
	
	
	$( "#easyView_comp" ).click(function() {
       

		$('#panel2ul_comp').html(packpremtext);
		//Button disable.
		//Per month
		$('#comppm1').html(rand+easyview+perMonth);
		//Permonth
		
		//Change Header Text and Color
		$('#dropdowntextcompack').html(easyviewtxt);
		$('#dropdownMenu3').attr("style","background-color:#d57d1c")
		
    });
	
	 //Compare Packages
	
	
	
	//Add Packages
	
	$( "#premium_comp_2" ).click(function() {
		//Change Device Panel Txt.
		$('#panel3_comp_add').html(addpackpremtext);
		//Change Device Panel Txt.
		
		//Per month
		$('#comppm2').html(rand+premium+perMonth);
		//Permonth
		
		//Change Header Text and Color
		$('#dropdowntextcomaddpack').html(premiumtxt);
		$('#dropdownMenu4').attr("style","background-color:#a5aaaf");
		$('#dropdowntextcomaddpack').attr("style","color:#ffffff");
      });
	
	
	
	
	$( "#extra_comp_2" ).click(function() {

		//Change Device Panel Txt.
		$('#panel3_comp_add').html(addpackextratext);
		//Change Device Panel Txt.
		
		//Per month
		$('#comppm2').html(rand+extra+perMonth);
		//Permonth
		
		//Change Header Text and Color
		$('#dropdowntextcomaddpack').html(extratxt);
		$('#dropdownMenu4').attr("style","background-color:#5faeab");
		$('#dropdowntextcomaddpack').attr("style","color:#ffffff");

      });
	
	
	$( "#compact_comp_2" ).click(function() {
        $(panel3).html(everywhereAccesstextfam);
		 
		 //Replace panel2 body text
		$("#panel3_comp_add").html(addpackcompacttext);
		//Replace panel2 body text
		
		//Per month
		$('#comppm2').html(rand+compact+perMonth);
		//Permonth
		
		//Change Header Text and Color
		$('#dropdowntextcomaddpack').html(compacttxt);
		$('#dropdownMenu4').attr("style","background-color:#f7dc06");
		$('#dropdowntextcomaddpack').attr("style","color:#ffffff");
    });
    
	
	$( "#family_comp_2" ).click(function() {
		
	
         $(panel3).html(everywhereAccesstextfam);
		 
		 //Replace panel2 body text
		$("#panel3_comp_add").html(addpackfamtext);
		//Replace panel2 body text
		
		//Per month
		$('#comppm2').html(rand+family+perMonth);
		//Permonth
		
		//Change Header Text and Color
		$('#dropdowntextcomaddpack').html(familytxt);
		$('#dropdownMenu4').attr("style","background-color:#84b341");
		$('#dropdowntextcomaddpack').attr("style","color:#ffffff");
    });
	
	$( "#access_comp_2" ).click(function() {
     
		 //Replace panel2 body text
		$("#panel3_comp_add").html(addpackAccesstext);
		//Replace panel2 body text
 
 
        //Per month
		$('#comppm2').html(rand+access+perMonth);
		//Permonth
		
		
		//Change Header Text and Color
		$('#dropdowntextcomaddpack').html(accesstxt);
		$('#dropdownMenu4').attr("style","background-color:#c30336");
        $('#dropdowntextcomaddpack').attr("style","color:#ffffff");
    });
	
	
	
	
	
	$( "#easyView_comp_2" ).click(function() {
       

		$("#panel3_comp_add").html(addpackEasytext);
		//Replace panel2 body text
		//Remove Disable attr
        //Per month
		$('#comppm2').html(rand+easyview+perMonth);
		//Permonth
		
		//Change Header Text and Color
		$('#dropdowntextcomaddpack').html(easyviewtxt);
		$('#dropdownMenu4').attr("style","background-color:#d57d1c");
		
		$('#dropdowntextcomaddpack').attr("style","color:#ffffff");
    });
	
	 //Add Packages
	
	
	
	
	
	
	
	/*
	$( "#extras" ).click(function() {
    $( extrasSum).toggle();
	 $( extrasSum2).toggle();
    });*/
	
	
});
    // JavaScript Document